...
GPU offloading for NVIDIA GPUs is automatically attempted as soon as a nhvpc compiler is detected. By default it will build for GPUs present on the host system. To cross compile for other architectures use `-DVASP_CUDA_ARCHITECTURES` (see below). If `MKLROOT` is set nvhpc will automatically link these for host side blas/lapack calls.
...
- `-DVASP_CUDA_ARCHITECTURES=<cc-versions list>`: list of nvidia compute capability / architectures. Just pass the numbers. Example `-DVASP_CUDA_ARCHITECTURES=100` for adding `-gpu=cc100` .
